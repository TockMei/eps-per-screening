...
st.caption("PEGスクリーニング｜PEG = PER ÷ 純利益CAGR(%)　｜　PEG < 1.0 → 利益の成長速度に対してPERが低い＝株価に成長が未反映")

# --- プリセット ---
PRESETS = {
    "📊 利益成長×PER乖離（デフォルト）": {"peg_max": 1.0, "cagr_min": 5.0, "mkt_min": 0, "period": "5y"},
    "🚀 高成長×超割安": {"peg_max": 0.5, "cagr_min": 15.0, "mkt_min": 0, "period": "5y"},
    "📈 成長割安（GARP）": {"peg_max": 1.0, "cagr_min": 10.0, "mkt_min": 0, "period": "5y"},
    "🏢 大型成長割安（時価総額1000億超）": {"peg_max": 1.0, "cagr_min": 8.0, "mkt_min": 1000, "period": "5y"},
    "⚡ 直近3年の勢い重視": {"peg_max": 1.0, "cagr_min": 10.0, "mkt_min": 0, "period": "3y"},
    "⚙️ カスタム": None,
}
...
if preset is not None:
    ...
    with c4:
        period = st.selectbox("CAGR期間", ["5y", "3y"], format_func=lambda x: "5年" if x == "5y" else "3年")

# --- テーブル構築 ---
df = build_table(period if preset is None else preset["period"])
...
with c5:
    period_label = "5年" if (period if preset is None else preset["period"]) == "5y" else "3年"
...
with chart_col1:
    ...
    ax.set_title(f"純利益成長率 × PER（線の下が割安）", fontsize=12)
...
st.caption(
    ...
    "CAGR: 純利益ベース（株式分割の影響を受けない）｜"
    "EDINET DB公式算出値を使用（eps_cagrは分割未調整のため純利益cagrを採用）｜"
    "時価総額: J-Quants（取得済み企業のみ）｜"
    "※EPSベースのCAGRは株式分割時に遡及調整されないため（EDINET XBRL仕様）、分割の影響を受けない純利益CAGRを採用しています"
)
